  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        {[
          { label: 'Pagi 06:00-14:00', color: 'bg-amber-100 text-amber-800 border-amber-200' },
          { label: 'Malam 14:00-22:00', color: 'bg-indigo-100 text-indigo-800 border-indigo-200' },
          { label: 'Libur', color: 'bg-gray-100 text-gray-500 border-gray-200' },
        ].map(l => (
          <span key={l.label} className={`px-2.5 py-1 rounded-full border font-medium ${l.color}`}>
            {l.label}
          </span>
        ))}
      </div>
                ...
                >
                  ...
                  <div className="mt-1 space-y-0.5">
                    {jadwalHari.map(j => {
                      ...
                      return (
                        <div key={j.id} className={`text-[10px] font-medium px-1.5 py-0.5 rounded-md truncate ${badge.color}`}>
                          {j.ob?.nama?.split(' ')[0]}
                        </div>
                      )
                    })}
